...
cleanup_redirect_chain6() {
    if [ "${HAS_NAT6:-0}" -eq 0 ] || [ "${HAS_REDIRECT6:-0}" -eq 0 ]; then
...
start_proxy() {
    ...
    if [ "$USE_TPROXY" -eq 1 ]; then
        setup_tproxy_chain4 || { rollback_proxy_start; return 1; }
        setup_routing4 || { rollback_proxy_start; return 1; }
        if [ "$PROXY_IPV6" -eq 1 ]; then
            setup_tproxy_chain6 || { rollback_proxy_start; return 1; }
            setup_routing6 || { rollback_proxy_start; return 1; }
        fi
    else
        setup_redirect_chain4 || { rollback_proxy_start; return 1; }
        if [ "$PROXY_IPV6" -eq 1 ]; then
            setup_redirect_chain6 || { rollback_proxy_start; return 1; }
        fi
    fi
...
block_ipv6_output() {
    case "$1" in
        enable)
            ...
            ip6tables -t filter -A OUTPUT -m owner --uid-owner 0 -j ACCEPT
