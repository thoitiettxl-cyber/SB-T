...
download_file() {
  ...
  if [ -f "$dest" ]; then
    if ! cp "$dest" "$bak"; then
      log Error "Failed to backup existing file: $dest"
      return 1
    fi
    had_backup=1
  fi
  ...
  if [ $ok -eq 0 ] || [ ! -s "$dest" ]; then
    ...
    if [ "$had_backup" -eq 1 ]; then
      if ! mv "$bak" "$dest"; then
        log Error "Failed to restore backup after download failure: $dest"
        return 1
      fi
    else
      rm -f "$dest"
...
do_upkernel() {
  ...
  mkdir -p "${BIN_DIR}/backup" || {
    log Error "Cannot create binary backup directory"
    return 1
  }
  if [ -x "$BIN_PATH" ]; then
    if ! cp "$BIN_PATH" "${BIN_DIR}/backup/sing-box.bak"; then
      log Error "Failed to back up current binary"
      return 1
    fi
  ...
  _restore_binary_backup() {
    if [ -f "${BIN_DIR}/backup/sing-box.bak" ]; then
      if ! cp "${BIN_DIR}/backup/sing-box.bak" "$BIN_PATH"; then
        log Error "Failed to restore binary from backup"
        return 1
      fi
    fi
    return 0
  ...
  if ! $busybox tar -xf "$tmp_tar" -C "$BIN_DIR" 2>/dev/null; then
    ...
    rm -f "$tmp_tar"
    _restore_binary_backup || return 1
    return 1
  fi
  rm -f "$tmp_tar"
  ...
  # setuidgid supplies the runtime identity; the downloaded binary itself does
  # not need setuid/setgid bits.
  chmod 0755 "$BIN_PATH"
