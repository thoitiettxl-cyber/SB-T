...
# BusyBox sed here uses "|" as the s/// delimiter (same pattern as CLIProxyAPI
# cpactl set_description). Any "|" inside the description breaks the rewrite
# with "bad option in substitution expression", so strip newlines and pipes.
_update_description() {
  ...
  if [ -f "$BOX_PID" ] && runtime_pid_is_core "$(cat "$BOX_PID" 2>/dev/null)" "$BIN_PATH"; then
    status="🟢 Running | SB Tproxy v${_ver} | tproxy :${TPROXY_PORT} | Action to stop"
  else
    status="🔴 Stopped | SB Tproxy v${_ver} | Action to start"
  fi
  safe=$(printf '%s' "$status" | tr '\n|' '  ')
  $busybox sed -i "s|^description=.*|description=${safe}|" "$MODULE_PROP" 2>/dev/null || true
}
