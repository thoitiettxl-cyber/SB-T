...
_kver=$(uname -r | command grep -oE '^[0-9]+\.[0-9]+' 2>/dev/null)
if handle_choice "Tải IPSET LKMs cho kernel ${_kver} không?" "Tải ngay" "Bỏ qua (tải sau)" 10; then
  ui_print "- Đang tải IPSET LKMs cho kernel ${_kver}..."
  export PATH="/data/adb/magisk:/data/adb/ksu/bin:/data/adb/ap/bin:$PATH:/system/bin"
  if /data/adb/box/scripts/upipset.sh; then
    ui_print "- Tải IPSET LKMs thành công"
  else
    ui_print "! Tải thất bại — sau khi reboot chạy: sbctl update-ipset"
  fi
fi
