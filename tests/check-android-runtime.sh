...
[ "$(jq -r '.dns.rules[0].domain[0]' "$REPO_ROOT/box/sing-box/config.json")" = "mdp-appconf-row.heytapdl.com" ] \
    || fail "exact noisy domain rule is not first"
[ "$(jq -r '.dns.rules[0].rcode' "$REPO_ROOT/box/sing-box/config.json")" = "NXDOMAIN" ] \
    || fail "exact noisy domain is not NXDOMAIN"
[ "$(jq -r '.inbounds[] | select(.tag == "tproxy-in") | .udp_timeout' "$REPO_ROOT/box/sing-box/config.json")" = "1m" ] \
    || fail "tproxy UDP timeout is not bounded to one minute"
...
root ". /data/adb/box/scripts/box.tool; . '${REPO_ROOT}/box/scripts/runtime.lib'; runtime_tproxy_port_ready '$pid' 1536 /data/adb/box/bin/sing-box 1" \
    || fail "TCP/UDP 1536 are not owned by the exact sing-box PID"
root ". /data/adb/box/scripts/box.tool; . '${REPO_ROOT}/box/scripts/runtime.lib'; SB_IPSET_BIN=/data/adb/box/bin/ipset; runtime_tproxy_state_ready /data/adb/box/runtime_tproxy.conf 1536" \
    || fail "module-owned tproxy state is incomplete"
root 'busybox flock -n 9 9>/data/adb/box/run/box.lock' \
    || fail "live sing-box retained lifecycle lock FD9"
...
pass "sing-box PID/socket ownership, tproxy state, lock, and listeners are healthy"
...
if command -v drill >/dev/null 2>&1; then
    ...
    heytap=$(drill mdp-appconf-row.heytapdl.com 2>&1)
    ...
    printf '%s\n' "$heytap" | grep -q 'rcode: NXDOMAIN' \
        || fail "exact HeyTap endpoint did not return NXDOMAIN"
    if printf '%s\n' "$heytap" | grep -q '198\.18\.'; then
        fail "exact HeyTap endpoint still returned FakeIP"
    fi
    printf '%s\n' "$direct_ad" | grep -q 'rcode: NXDOMAIN' \
        || fail "direct port-53 query escaped DNS hijack"
    pass "exact HeyTap NXDOMAIN, DNS ad blocking, and port-53 interception work"
else
